...
export async function PUT(
  ...
  try {
    ...
    const { name, description, minScore, maxScore, displayOrder, weight, category } = await request.json()
    ...
    const [currentCriterion] = await db.select().from(criteria).where(eq(criteria.id, criterionId)).limit(1)
