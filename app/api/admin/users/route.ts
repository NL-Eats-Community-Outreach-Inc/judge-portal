...
export async function GET() {
  try {
    ...
    // 3. Participants on teams in this org's events (teamMembers → teams → events)
    const orgParticipantRows = await db
      .selectDistinct({ participantId: teamMembers.participantId })
      .from(teamMembers)
      .innerJoin(teams, eq(teams.id, teamMembers.teamId))
      .innerJoin(events, eq(events.id, teams.eventId))
      .where(eq(events.organizationId, orgId));
    const orgParticipantIds = orgParticipantRows.map((r) => r.participantId);
