...
export function TeamScoringInterface({
  ...
  const { isFullyComplete, event, hasShownCompletionConfetti, setHasShownCompletionConfetti } =
  ...
  const handleScoreChange = (criterionId: string, newScore: number) => {
    ...
    setScores((prev) =>
      prev.map((s) => (s.criterionId === criterionId ? { ...s, score: newScore } : s))
    );
  ...
  const handleCommentChange = (criterionId: string, newComment: string) => {
    ...
    // Update UI immediately
    setScores((prev) =>
      prev.map((s) => (s.criterionId === criterionId ? { ...s, comment: newComment } : s))
    );
  };
      ...
      <div className="prose prose-xs dark:prose-invert max-w-none text-xs">
        <ReactMarkdown
          components={{
            ...
            ul: ({ children }) => (
              <ul className="mb-1.5 ml-3 list-disc last:mb-0 text-xs">{children}</ul>
            ),
            ol: ({ children }) => (
              <ol className="mb-1.5 ml-3 list-decimal last:mb-0 text-xs">{children}</ol>
            ),
    ...
    if (isTouchDevice) {
      ...
      return (
        <div className="flex items-start gap-2">
          <div className="flex-1">{truncatedDescription}</div>
          <Popover>
      ...
      return (
        <Tooltip>
          <TooltipTrigger asChild>
            <div className="cursor-help">{truncatedDescription}</div>
          </TooltipTrigger>
            ...
            return (
              <Button
                ...
                className={cn(
                  ...
                  isSelected &&
                    'ring-2 ring-ring ring-offset-1 md:ring-offset-2 bg-gradient-to-r from-primary to-primary/90'
                )}
              >
                {value}
              </Button>
            ...
            return (
              <Button
                ...
                className={cn(
                  ...
                  isSelected &&
                    'ring-2 ring-ring ring-offset-1 bg-gradient-to-r from-primary to-primary/90'
                )}
              >
                {value}
              </Button>
  ...
  return (
    <div className="p-4 md:p-6 max-w-4xl mx-auto">
      <div className="space-y-4 md:space-y-6">
            ...
            <div className="flex items-start justify-between min-w-0">
              <div className="min-w-0 flex-1">
                ...
                <div className="flex flex-wrap gap-2 mt-2">
                  <Badge variant="outline">Presentation #{team.presentationOrder}</Badge>
        ...
        {/* Scoring criteria */}
        <div className="space-y-3 md:space-y-4">
          <h2 className="text-lg md:text-xl font-semibold text-foreground">Scoring Criteria</h2>
        ...
        <Card className="p-3 md:p-4">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div>
              ...
              <p className="text-xs text-muted-foreground">Your scores are saved automatically</p>
            </div>
      ...
      <CompletionConfetti show={showConfetti} onComplete={() => setShowConfetti(false)} />
    </div>
  );
}
