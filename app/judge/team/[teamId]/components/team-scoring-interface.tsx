...
export function TeamScoringInterface({ 
  ...
  // Set up real-time sync for criteria table
  useRealtimeTable('criteria', () => {
    fetchExistingScores()
  }, { enabled: true })

  // Set up real-time sync for events table (to detect when event becomes inactive)
  useRealtimeTable('events', () => {
    // Trigger a navigation to force server component re-evaluation
