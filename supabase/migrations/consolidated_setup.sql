...
--
-- Fresh databases only: `drizzle-kit push` (run first by scripts/consolidated-setup.ts)
-- creates the tables, enums, constraints and indexes from lib/db/schema.ts; this
-- file adds what the schema cannot describe: the functions, the auth trigger, RLS
-- and the policies (plus the indexes below, guarded, for a database that never had
-- the push). The CREATE TABLE statements are guarded and never run after the push;
-- they mirror lib/db/schema.ts so this file does not describe a schema that does
-- not exist. Every statement is idempotent; the file runs as one transaction.

-- ================================================================
-- STEP 1: CREATE ENUM TYPES
-- ================================================================

-- The enums are created by `drizzle-kit push` (scripts/consolidated-setup.ts
-- runs it first), so each CREATE TYPE is guarded: an unguarded one would fail
-- and abort everything after it in this file.
...
-- User role enum (001_invite-link and 002_multi-tenant-participants add the other roles)
DO $$ BEGIN
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'criteria_category') THEN
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'team_award_type') THEN
...
DROP POLICY IF EXISTS "Judges can view own assignments" ON event_judges;
CREATE POLICY "Judges can view own assignments" 
  ON event_judges FOR SELECT 
  USING (auth.uid() = judge_id);

DROP POLICY IF EXISTS "Admins can view all assignments" ON event_judges;
...
DROP POLICY IF EXISTS "Admins can insert assignments" ON event_judges;
...
DROP POLICY IF EXISTS "Admins can delete assignments" ON event_judges;
...
DROP POLICY IF EXISTS "Judges can view own scores" ON scores;
CREATE POLICY "Judges can view own scores" 
  ON scores FOR SELECT 
  USING (auth.uid() = judge_id);

DROP POLICY IF EXISTS "Judges can insert own scores" ON scores;
...
DROP POLICY IF EXISTS "Judges can manage their own scores" ON scores;
...
DROP POLICY IF EXISTS "Judges can view all events" ON events;
...
DROP POLICY IF EXISTS "Judges can view teams" ON teams;
...
DROP POLICY IF EXISTS "All authenticated users can view criteria" ON criteria;
CREATE POLICY "All authenticated users can view criteria" 
  ON criteria FOR SELECT 
  USING (auth.uid() IS NOT NULL);

DROP POLICY IF EXISTS "Judges can view criteria" ON criteria;
...
-- submissions and submission_ai_scores keep row level security enabled with no
-- client policies: the feature is disabled (SUBMISSIONS_ENABLED) and the server
-- reaches both tables through the database client only. Production has none.
