...
export const teams = pgTable(
  ...
  (table) => ({
    ...
    awardTypeIdx: index('idx_teams_award_type').on(table.awardType),
  })
);
...
export const criteria = pgTable(
  'criteria',
  {
    ...
    category: criteriaCategoryEnum('category').default('technical').notNull(),
    createdAt: timestamp('created_at', { withTimezone: true, mode: 'string' })
      .default(sql`timezone('utc'::text, now())`)
      .notNull(),
    updatedAt: timestamp('updated_at', { withTimezone: true, mode: 'string' })
  ...
  (table) => ({
    ...
    categoryIdx: index('idx_criteria_category').on(table.category),
  })
);
...
export const eventJudges = pgTable(
  ...
  (table) => ({
    pk: primaryKey({ columns: [table.eventId, table.judgeId] }),
...
export const submissions = pgTable(
  ...
  (table) => ({
    uniqueEventTeam: unique().on(table.eventId, table.teamId),
  })
);
